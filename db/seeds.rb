...
rand(1..20).times do
  ...
  new_event.save!
  items = ['tente', 'gourde', 'pain', 'fruits', 'chocolat', 'red bull x monster de la mort qui tue gout piment']
  items.each do |item|
    Item.create(name: item, quantity: groupsize + rand(1..10), event: new_event)
  end
  rand(0..1).times do
